...
- **מסך כניסה לתרגול** (`#/`) — המסך הראשון בכל כניסה רגילה. הוא מסביר בעברית פשוטה את ההבדל בין תרגול חופשי ללא רישום, שבו הציון מוצג אך אינו נשמר, לבין חשבון רשום שבו ציונים והתקדמות נשמרים. מתחת למסך זה ממשיך אתר חומרי הלימוד הקיים.
- **אתר חומרי הלימוד וסרט הפתיחה** (`#/home`) — סרט הרכבת הקלה בירושלים, סרטון עדכון ת״ל, חוברת העבודה והכניסה לחומרים. אותו תוכן נטען גם מתחת למסך הכניסה כדי לא לשבור את חוויית האתר הקיימת.
- **התפריט** (`#/menu`) — תצוגה · הורדה · הדפסה · וואטסאפ, ובוחר עמודים.
- **דפי העבודה** (`#/workbook/:n`) — 78 עמודים, ניווט עמוד־אחר־עמוד, זום, שמירת טיוטה ומספר ניסיונות והדפסת עמוד בודד. במצב אורח ציוני עמוד אינם נשמרים.
...
src/
  ...
  games/                      7 המשחקים + מנוע "גילוי מילה/קוד" משותף
  views/                      welcome · home · menu · pageViewer · book ·
                              coverSheet · tocSheet · printBar · מסכי LMS
