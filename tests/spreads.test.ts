...
/* The spread model is the flipbook's arithmetic: which page sits on which half,
   ...
   ours against our totals (80 = cover + contents + 78 pages). */
...
describe('the flipbook spread model', () => {
  ...
  it('the last open spread pairs the final page with the colophon', () => {
    ...
    expect(pagesAtSpread(last, TOTAL)).toEqual({ right: { kind: 'page', page: 80 }, left: { kind: 'back-inner' } });
    // …and the two pages before it still sit together on the spread before.
