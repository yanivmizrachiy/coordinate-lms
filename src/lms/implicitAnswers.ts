...
import { hydrateNonRightAngleExample } from './digitalNonRightAngleExample';
...
export function hydrateExplicitAuthoringAnswers(root: ParentNode): () => void {
  ...
  const cleanupNonRightAngle = hydrateNonRightAngleExample(root);
  ...
  const cleanupEncryptedRoute = hydrateDigitalEncryptedRoutePredicate(root);
