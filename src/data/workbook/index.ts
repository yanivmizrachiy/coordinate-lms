/* ===========================================================================
   ...
   (31.07.2026), so the interactive games became printed pages too and no
   CANONICAL page mounts a widget. Page numbers are assigned by POSITION, so a
   page can be inserted, split or reordered without hand-editing the number
   baked in.

   LMS layer: the printed puzzle pages stay the canonical print sheets; ON
   SCREEN the matching interactive game is mounted alongside by pageViewer,
   through the `gameId` attached in SCREEN_GAMES below. The attachment lives
   here — never inside a page file — so the canonical content stays pure print.
   =========================================================================== */
import {
  AXES_IDENTIFY,
...
/* LMS-only: which interactive game plays ON SCREEN on each printed puzzle
   page. The paper stays exactly the canonical worksheet; the screen adds the
   game as a layer (pageViewer mounts it in a no-print host). One map, here,
   so a content file can never fork into a second „interactive version". */
const SCREEN_GAMES = new Map<WorkbookPageContent, string>([
  [HIDDEN_DRAWING_PRINT, 'hidden-drawing'],
  [COLOR_DECODE_PRINT, 'color-decode'],
  [ENCRYPTED_ROUTE_PRINT, 'encrypted-route'],
  [COORDINATE_MAZE_PRINT, 'coordinate-maze'],
  [COORDINATE_SAFE_PRINT, 'coordinate-safe'],
  [SUSPECT_POINT_PRINT, 'suspect-point'],
  [SAME_AXIS_PRINT, 'same-axis'],
]);
...
export const WORKBOOK: WorkbookPageContent[] = ORDER.map(({ slot, chapterTitle }, i) => {
  const page = renumber(chapterTitle ? rechapter(slot, chapterTitle) : slot, i + 1);
  const gameId = SCREEN_GAMES.get(slot);
  return gameId ? { ...page, gameId } : page;
});
